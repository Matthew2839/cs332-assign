...
object Huffman {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    (makeCodeTree(trees.head, trees.tail.head)::trees.tail.tail).sortBy(tree => weight(tree))
  }
   ...
   */
  def until(fa: List[CodeTree] => Boolean,
            fb: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    if (fa(trees)) trees
    else until(fa, fb)(fb(trees))
  }
      ...
      if (bits == Nil) ans ::: chars(tree)
        ...
        tree match {
          case Fork(left, right, _, _) => {
            if(bits.head == 0) iter(left, bits.tail, ans)
            else iter(right, bits.tail, ans)
          }
          case Leaf(char, _) => iter(origin ,bits, ans ::: List(char))
        }
      }
    }
    iter(tree, bits, List())
  ...
  def decodedSecret: List[Char] = {
    val answer = decode(frenchCode, secret)
    print(answer)
    answer
  }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def encodeLoop(table: CodeTable, text: List[Char]): List[Bit] = {
      if (text == Nil) Nil
      else codeBits(table)(text.head) ::: encodeLoop(table, text.tail)
    }
    encodeLoop(codeTable, text)
  }
}
